-- ============================================================
-- Mythsensus — Supabase (PostgreSQL) Schema v2
...
-- 0. EXTENSIONS
-- ============================================================
-- FIX: SEC-H03 — Enable pgcrypto for PII encryption
...
CREATE TABLE public.profiles (
  ...
  name               text        NOT NULL,
  dob                date        NOT NULL,
  birth_time         text,       -- FIX: SEC-H03 — changed to text for encrypted storage
  birth_place        text,       -- FIX: SEC-H03 — will store encrypted values
  gender             text        CHECK (gender IN ('male', 'female', 'other')),
  relationship_label text        NOT NULL DEFAULT 'myself',
  cosmic_score       integer     CHECK (cosmic_score >= 0 AND cosmic_score <= 1000),
  created_at         timestamptz NOT NULL DEFAULT now(),
  -- FIX: SEC-C03 — Data retention policy
...
CREATE TABLE public.reports (
  ...
  -- FIX: SEC-C02 — Store storage path, not public URL
  pdf_storage_path text,
  -- FIX: SEC-C02 — Access token for signed URL verification
...
-- FIX: SEC-L08 — Audit log table for sensitive operations
...
-- FIX: SEC-M06 — Restrict UPDATE so users CANNOT change their own plan column
...
-- FIX: SEC-H01 — Add DELETE policy for PDPA Right to Erasure
...
-- FIX: SEC-H05 — Added SET search_path = public
...
-- ============================================================
-- 6. FIX: SEC-M06 — Secure plan change function
-- ============================================================
...
-- ============================================================
-- 7. FIX: SEC-H02 — Rate limiting functions
-- ============================================================
...
COMMENT ON FUNCTION public.fn_check_god_draw_limit() IS
  'SEC-H02: Rate limit god draws — 1/day free, 3/day subscriber.';
...
COMMENT ON FUNCTION public.fn_check_organum_limit() IS
  'SEC-H02: Rate limit organum sessions — 5/day free, unlimited subscriber.';


-- ============================================================
-- 8. FIX: SEC-H03 — PII Encryption helpers (pgcrypto)
-- ============================================================
...
-- ============================================================
-- 9. FIX: SEC-C02 — PDF Signed URL helper
-- ============================================================
...
-- ============================================================
-- 10. FIX: SEC-C03 — Data retention cleanup function
-- ============================================================
...
BEGIN
  ...
  -- Clean up old frequency_alerts (older than 90 days) — FIX: SEC-L04
...
COMMENT ON FUNCTION public.fn_cleanup_expired_data() IS
  'SEC-C03 + SEC-L04: Periodic data cleanup — removes expired profiles, old alerts, old audit logs. Call via pg_cron or Supabase scheduled function.';


-- ============================================================
-- 11. FIX: SEC-H01 — User self-deletion (Right to Erasure)
-- ============================================================
