...
const ck = { chart_hash: hash, system: '_grid', lang: 'th', relationship_status: 'unknown', prompt_version: 'gridtest-20260904b' }
...
async function fire(callSet, label) {
  ...
  for (let i = 0; i < 45; i++) {
    ...
    const rows = await (await fetch(REST + '/myth_addon_reading?chart_hash=eq.' + hash + '&prompt_version=eq.' + encodeURIComponent(ck.prompt_version) + '&select=phase,cost_cents,oracle_json', { headers: sb })).json()
    const row = rows[0]
    if (row && row.oracle_json && row.oracle_json.cells) {
      // แถวเปลี่ยนแล้ว = รอบนี้เขียนเสร็จ
      const jobs = await (await fetch(REST + '/oracle_render_jobs?job_key=eq.' + encodeURIComponent(jobKey) + '&select=status', { headers: sb })).json()
      if (row.phase === 'grid' || (jobs[0] && jobs[0].status === 'grid_partial')) return row
    }
...
for (let round = 2; round <= 3 && row.phase !== 'grid'; round++) {
  const g = row.oracle_json.grid || {}
  const missing = []
  for (const sys of Object.keys(g)) for (const q of questions)
    if (g[sys][q.q] === undefined) missing.push({ sys, q })
  if (!missing.length) break
  console.log('')
  console.log('ขาด', missing.length, 'ช่อง — ยิงรอบเก็บตกที่', round)
  ...
  const topUp = Object.values(byQ).slice(0, 20).map((e, i) => {
    ...
    return {
      key: 'top' + round + '_' + i,
      maxTokens: Math.min(16000, e.sys.length * 150 + 900),
      systemPrompt: [
        ...
        '',
        'กติกา',
        '- ตอบเฉพาะศาสตร์ที่อยู่ในข้อมูล ห้ามเพิ่มศาสตร์อื่น',
