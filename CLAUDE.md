...
- **Payment:** Gumroad (current). `_GUMROAD_SUBSCRIBE_URL` at index.html ~L4787 points at the active product. Buyer flow opens Gumroad checkout → Gumroad pings `/api/gumroad/webhook`, which (a) for the subscription product (`tlkfx`) sets `auth.users.app_metadata.plan = 'premium'` via the admin API (best-effort mirror to `public.users.plan`), and (b) for per-item products upserts a row into `public.myth_purchases` keyed by `sale_id`. `/api/me/plan` reads `auth.users.app_metadata.plan`; `/api/me/purchases` reads `myth_purchases` (service-role, email-scoped). Migration arc: Stripe (rejected — Thai entity) → LemonSqueezy (briefly live mid-2026-05) → Gumroad (2026-05-28+, current). Old `api/lemonsqueezy/*` endpoints may still exist as historical files but are no longer called.
- **Backend:** Vercel serverless functions under `api/`. Active endpoints:
  ...
  - `api/gumroad/webhook.js`            — Gumroad webhook → sets `app_metadata.plan` (subscription) + upserts `myth_purchases` (per-item)
  - `api/me/plan.js`                    — Returns logged-in user's `auth.users.app_metadata.plan` for client gating
  - `api/me/purchases.js`               — Returns logged-in user's per-item unlocks from `myth_purchases`
...
- Subscription state lives in `auth.users.app_metadata.plan` (set by `api/gumroad/webhook.js` via the admin API). The `myth_profiles` / `myth_subscriptions` / `myth_orders` tables defined in `migrations/001_auth_payment_schema.sql` were **never created on woam** — the file is deprecated and kept only as historical reference. (Verified 2026-06-02: re-checking woam confirmed all four `myth_*` tables from 001 were MISSING; the codebase never imports them.)
- Per-item purchases (Deep Reading, Mirror, Pet, ...) are recorded in `public.myth_purchases` by the same webhook, and read back by `api/me/purchases.js`. Applied via `migrations/002_purchases.sql` on **2026-06-02** through the Supabase dashboard SQL editor (driven by Claude-in-Chrome extension on Pattrick's logged-in session). Before this, every per-item Gumroad sale silently failed at the webhook write step → buyers lost cross-device unlock for those items (the `/api/me/purchases` endpoint has a graceful `note: 'lookup_failed'` fallback so the app never errored — it just returned `items: []`). Pre-2026-06-02 sales can be backfilled by replaying Gumroad `/v2/sales` against the webhook.
