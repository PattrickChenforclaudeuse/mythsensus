...
export default async function handler(req, res) {
  ...
  if (error) return res.redirect('/onboarding/?error=google_denied');
  ...
  try {
    ...
    if (!tokenData.access_token) return res.redirect('/onboarding/?error=token_failed');
    ...
    if (SUPABASE_URL && SUPABASE_SERVICE_KEY) {
      const upsertResp = await fetch(SUPABASE_URL + '/rest/v1/users', {
        ...
        body: JSON.stringify({ google_user_id: profile.id, display_name: profile.name, email: profile.email, picture_url: profile.picture || null, google_access_token: tokenData.access_token, updated_at: new Date().toISOString() })
      });
      const upsertData = await upsertResp.json();
      if (Array.isArray(upsertData) && upsertData[0]) userId = upsertData[0].id;
    }
    const sessionPayload = { provider: 'google', google_user_id: profile.id, display_name: profile.name, email: profile.email, picture_url: profile.picture, user_id: userId, exp: Date.now() + (7 * 24 * 60 * 60 * 1000) };
    const sessionToken = Buffer.from(JSON.stringify(sessionPayload)).toString('base64');
    res.setHeader('Set-Cookie', ['ms_session=' + sessionToken + '; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=' + (7 * 24 * 60 * 60)]);
